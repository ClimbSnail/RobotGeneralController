...
class RobotSocketClient(RobotSocket):
    ...
    def start(self):
        ...
        run_thread = threading.Thread(target=reconner, args=())
        run_thread.start()
        run_thread = threading.Thread(target=self.recvfrom_ser, args=())
        run_thread.start()
...
if __name__ == "__main__":
    ...
    sersocket = RobotSocketServer("192.168.123.135", 5555, myRecvHandle, max_bind=10)
    ...
    def myRecvHandle(dat):  # 接收函数
        ...
        print(dat)
    # 初始化端口并设置接收数据的函数(当接收到数据，自动被调用)
    clientsocket = RobotSocketClient("192.168.123.135", 5555, myRecvHandle)
